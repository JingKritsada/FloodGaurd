			...
			<div className="flex flex-col h-full bg-white dark:bg-slate-950">
				<div className="flex-none p-4 flex items-center justify-between">
					...
					<h2 className="font-black text-lg dark:text-white">
						{reportType === "SOS" ? "จุดขอความช่วยเหลือ" : "จุดแจ้งเหตุ"}
					</h2>
					<div className="w-20" />
				</div>
				<div className="absolute top-20 inset-x-6 z-[400] flex justify-center pointer-events-none">
					<div className="bg-slate-900/80 text-white px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest shadow">
		...
		<div className="p-6 bg-white dark:bg-slate-950 h-full overflow-y-auto pb-24 space-y-5">
			<div className="flex items-center gap-4">
				...
				<h2 className="text-xl font-black text-slate-900 dark:text-white">
					{reportType === "SOS" ? "รายละเอียดขอความช่วยเหลือ" : "รายละเอียดการแจ้งเหตุ"}
				</h2>
			</div>

			<FormMap draggable={false} height="h-40" singlePoint={formLocation} />

			{reportType === "SOS" && (
				<>
					<div>
						<p className="block text-sm font-black text-slate-900 dark:text-white mb-3">
					...
					<div>
						...
						<label className="flex items-center gap-3 mt-3 p-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl cursor-pointer">
							...
							<span className="text-sm font-bold text-slate-700 dark:text-slate-300">
								มีผู้ป่วยติดเตียง / เคลื่อนย้ายลำบาก
							</span>
						</label>
					</div>
				</>
			...
			<div>
				<label
					className="block text-sm font-black text-slate-400 uppercase tracking-widest mb-3"
					htmlFor="report-desc"
				>
					รายละเอียดสถานการณ์
				</label>
				<textarea
					className="w-full p-5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl text-slate-900 dark:text-white font-medium text-base focus:ring-4 focus:ring-amber-500/10 outline-none resize-none"
					id="report-desc"
			...
			<div>
				...
				<div className="relative">
					...
					/>
					<input
						className="w-full pl-12 pr-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl text-slate-900 dark:text-white font-medium text-base focus:ring-4 focus:ring-amber-500/10 outline-none"
						id="report-phone"
			...
			<div>
				...
				<input
					className="w-full p-5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl text-slate-900 dark:text-white font-medium text-base focus:ring-4 focus:ring-amber-500/10 outline-none"
					id="report-address"
					placeholder="หมู่ 1, ต.เมืองเก่า, อ.เมือง"
					type="text"
					value={address}
