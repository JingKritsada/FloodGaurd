...
export default function OfficerReportSection({
	officerReportMode,
	formLocation,
	draftPoints,
	formDesc,
		...
		return (
			<div className="p-6 bg-slate-50 dark:bg-slate-950 h-full overflow-y-auto flex flex-col items-center justify-start space-y-8">
				...
				<div className="w-full max-w-sm space-y-4">
					{[
						{
							mode: "ROAD_CLOSURE" as OfficerReportMode,
						...
						{
							mode: "LEVEE" as OfficerReportMode,
							icon: <Waves size={28} />,
							label: "น้ำล้น/ตลิ่งพัง",
							sub: "Critical Point",
							color: "bg-blue-500",
						},
						{
							mode: "GENERAL" as OfficerReportMode,
							icon: <AlertTriangle size={28} />,
							label: "เหตุอุบัติภัยทั่วไป",
							sub: "Rescue Support",
							color: "bg-amber-500",
						},
					].map((item) => (
						<button
							...
							onClick={() => setOfficerReportMode(item.mode)}
						>
							<div
							...
							<div>
								<span className="font-black text-lg text-slate-900 dark:text-white block leading-tight">
									{item.label}
								</span>
	...
	// Map drawing screen
	const needsMap =
		(isRoad && !formDesc.trim()) ||
		(isLevee && !formLocation) ||
		(officerReportMode === "GENERAL" && !formLocation);
			...
			<div className="flex flex-col h-full bg-white dark:bg-slate-950">
				...
				<div className="flex-1 relative">
					<FormMap
						...
						height="h-full"
						onMapClick={(loc) => {
							if (isRoad) setDraftPoints((prev) => [...prev, loc]);
		...
		<div className="p-6 bg-white dark:bg-slate-950 h-full overflow-y-auto pb-24 space-y-5">
			<div className="flex items-center gap-4">
				<button
					...
					onClick={() => {
						if (isRoad) setFormDesc("");
						else setFormLocation(null);
					}}
				>
					<ArrowLeft size={20} />
				</button>
			...
			<FormMap
				draftPoints={isRoad ? draftPoints : []}
				draggable={false}
				height="h-40"
				singlePoint={formLocation ?? undefined}
			/>

			<div>
				<label className="block text-sm font-black text-slate-400 uppercase tracking-widest mb-3">
					{isRoad ? "สาเหตุ / สถานะ" : "รายละเอียดสถานการณ์"}
				</label>
				<textarea
					...
					placeholder={
						isRoad ? "น้ำท่วมสูง... รถเล็กผ่านไม่ได้..." : "อธิบายสถานการณ์..."
					}
					rows={3}
					value={formDesc.trim()}
