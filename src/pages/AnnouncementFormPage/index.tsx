...
const PRIORITY_OPTIONS = [
	{ id: "HIGH" as const, label: "ด่วนมาก", icon: <AlertTriangle size={18} />, color: "bg-red-500" },
...
export default function AnnouncementFormPage() {
	const { isEdit, loading, title, setTitle, content, setContent, priority, setPriority, image, setImage, isPublished, setIsPublished, handleSubmit, handleCancel } = useAnnouncementForm();
	...
	return (
		<div className="bg-white dark:bg-slate-950 h-full overflow-y-auto pb-24">
			<div className="sticky top-0 bg-white/80 dark:bg-slate-950/80 backdrop-blur-lg z-10 px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex items-center gap-4">
				<button onClick={handleCancel} className="p-3 bg-slate-100 dark:bg-slate-900 rounded-2xl text-slate-600 dark:text-slate-400 hover:bg-amber-500 hover:text-white transition-all">
					<ArrowLeft size={20} />
				</button>
				<h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{isEdit ? "แก้ไขประกาศ" : "สร้างประกาศใหม่"}</h2>
			</div>
			...
			<div className="p-6 space-y-6">
					...
					<div className="grid grid-cols-3 gap-3">
						{PRIORITY_OPTIONS.map((p) => (
							<button key={p.id} onClick={() => setPriority(p.id)}
								...
								<div className={`p-2 rounded-xl ${priority === p.id ? "bg-white/20" : "bg-white dark:bg-slate-800 shadow-sm"}`}>
									{(p.icon as React.ReactElement<{ className: string }>).type
										? <p.icon.type {...(p.icon as React.ReactElement<{ className: string }>).props} className={priority === p.id ? "text-white" : "text-slate-600 dark:text-slate-400"} />
										: p.icon}
								</div>
								<span className={`text-xs font-bold text-center ${priority === p.id ? "text-white" : "text-slate-900 dark:text-slate-300"}`}>{p.label}</span>
							</button>
						))}
					</div>
				</div>
				...
				<div>
					...
					<textarea value={content} onChange={(e) => setContent(e.target.value)}
						...
						rows={6} placeholder="รายละเอียดประกาศ..." />
				</div>
				...
				<div>
					...
					{image && <div className="mt-3 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-800"><img src={image} alt="Preview" className="w-full h-48 object-cover" /></div>}
				</div>

				<div>
					<label className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl cursor-pointer hover:border-amber-400 transition-all">
						<input type="checkbox" checked={isPublished} onChange={(e) => setIsPublished(e.target.checked)} className="w-5 h-5 rounded border-2 border-slate-300 dark:border-slate-600 text-amber-600 cursor-pointer" />
						<div>
				...
				<button onClick={handleSubmit} className="w-full py-5 rounded-3xl font-black text-base uppercase tracking-widest text-white bg-amber-600 shadow-2xl shadow-amber-600/30 active:scale-95 transition-all flex items-center justify-center gap-3">
