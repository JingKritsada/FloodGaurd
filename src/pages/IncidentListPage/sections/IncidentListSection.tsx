...
export default function IncidentListSection({
	incidents,
	role,
	onStatusUpdate,
	onViewOnMap,
		...
		<div className="bg-slate-50 dark:bg-slate-950 h-full overflow-y-auto pb-32">
			...
			<div className="p-4 space-y-4">
				{filtered.map((inc) => (
					<div
						key={inc.id}
						className="bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-xl hover:scale-[1.01] transition-all group"
					>
						<div className="flex gap-4">
							<div
								className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-sm ${statusColorMap[inc.status]}`}
							>
				...
				{filtered.length === 0 && (
					<div className="text-center py-20">
						<div className="w-20 h-20 bg-slate-100 dark:bg-slate-900 rounded-full flex items-center justify-center mx-auto mb-6">
							<List className="text-slate-300 dark:text-slate-700" size={32} />
						</div>
