...
function createIcon(iconNode: React.ReactNode, bgColor: string, pulse = false) {
	const html = ReactDOMServer.renderToString(
		<div
			className={`relative flex items-center justify-center w-10 h-10 rounded-xl border-2 border-white shadow-xl ${bgColor} ${pulse ? "animate-pulse" : ""}`}
		>
			{iconNode}
		</div>
	);
...
function getIncidentIcon(type: string, status: string) {
	const iconMap: Record<string, React.ReactNode> = {
		MEDICAL: <Stethoscope className="text-white" size={20} />,
		EVACUATION: <Truck className="text-white" size={20} />,
		SUPPLIES: <Package className="text-white" size={20} />,
		ROAD_BLOCKED: <AlertOctagon className="text-white" size={20} />,
		LEVEE_BREACH: <Waves className="text-white" size={20} />,
	};
	...
	const icon = iconMap[type] ?? <AlertTriangle className="text-white" size={20} />;
	const color = colorMap[type] ?? "bg-slate-500";

	return createIcon(icon, color, status === "OPEN");
}

const shelterIcon = createIcon(<Home className="text-white" size={18} />, "bg-amber-500");
...
function MapController({ userPosition }: { userPosition: Location | null }) {
	...
	return (
		<div
			ref={controlsRef}
			className="absolute bottom-28 right-4 flex flex-col gap-3 z-[1000] pointer-events-auto"
		>
			<button
				...
				type="button"
				onClick={() => map.flyTo([DEMO_CENTER.lat, DEMO_CENTER.lng], 13, { duration: 1.2 })}
			>
				<RefreshCw size={20} />
			</button>
			<button
...
export default function MapSection({
	...
	const filteredIncidents =
		filter === "ALL"
			...
			: filter === "SOS"
				? incidents.filter((i) =>
						["MEDICAL", "SUPPLIES", "EVACUATION", "RISK_AREA"].includes(i.type)
					)
				: filter === "TRAFFIC"
				...
				{showRoads &&
					roads.map((road) => (
						<Polyline
							...
							positions={(
								road.path ?? [
									{ lat: road.start.lat, lng: road.start.lng },
									{ lat: road.end.lat, lng: road.end.lng },
								]
						...
						.map((inc) => (
							<Polyline
								...
								pathOptions={{
									...
									weight: 6,
									opacity: 0.8,
									dashArray: inc.status === "OPEN" ? "10, 10" : undefined,
								}}
								positions={(inc.path ?? []).map(
				...
				{filteredIncidents
					...
					.map((inc) => (
						<Marker
							key={inc.id}
							eventHandlers={{ click: () => onMarkerClick(inc) }}
							icon={getIncidentIcon(inc.type, inc.status)}
