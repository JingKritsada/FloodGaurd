				...
				<div className="flex justify-between items-start mb-8">
					<div className="space-y-2">
						<div className="flex items-center gap-2">
							<span
								className={`text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-widest ${statusColors[incident.status] ?? "bg-slate-500 text-white"}`}
							>
								{STATUS_LABELS[incident.status] ?? incident.status}
							</span>
							<span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
								Case #{incident.id.slice(-4)}
							</span>
						</div>
					...
					<button
						className="p-3 bg-slate-100 dark:bg-slate-800 hover:bg-amber-500 hover:text-white rounded-full text-slate-400 transition-all"
						onClick={onClose}
					>
						<X size={24} />
					</button>
