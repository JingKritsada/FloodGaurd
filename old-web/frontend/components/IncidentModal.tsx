...
import { Incident, Role, TicketStatus } from '../types';
import { TYPE_LABELS, STATUS_LABELS } from '../constants';
import { X, User, List, Map as MapIcon, Calendar, Clock, Navigation } from 'lucide-react';
...
const IncidentModal: React.FC<IncidentModalProps> = ({ incident, onClose, role, onStatusUpdate }) => {
  if (!incident) return null;

  return (
    <div className="fixed inset-0 z-[2000] flex items-end md:items-center justify-center pointer-events-none p-0 md:p-6">
      <div className="absolute inset-0 bg-slate-950/40 pointer-events-auto transition-all" onClick={onClose}></div>
      <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-t-[40px] md:rounded-[40px] shadow-2xl p-8 pointer-events-auto transform transition-all animate-in slide-in-from-bottom duration-500 ease-out border-t md:border border-slate-200 dark:border-slate-800">
        ...
        <div className="flex justify-between items-start mb-8">
           <div className="space-y-2">
             <div className="flex items-center gap-2">
               <span className={`text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-widest ${
                 ...
                 incident.status === 'IN_PROGRESS' ? 'bg-amber-500 text-white' :
           ...
           <button onClick={onClose} className="p-3 bg-slate-100 dark:bg-slate-800 hover:bg-gold-500 hover:text-white rounded-full text-slate-400 transition-all">
             <X size={24} />
           </button>
        </div>
        ...
        <div className="space-y-6 mb-10">
          <div className="grid grid-cols-2 gap-4">
             <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-3xl border border-slate-100 dark:border-slate-800">
               <div className="flex items-center gap-2 text-slate-400 mb-1">
                 <Calendar size={14} />
                 <span className="text-[10px] font-bold uppercase tracking-widest">วันที่แจ้ง</span>
               </div>
             ...
             <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-3xl border border-slate-100 dark:border-slate-800">
               <div className="flex items-center gap-2 text-slate-400 mb-1">
                 <Clock size={14} />
                 <span className="text-[10px] font-bold uppercase tracking-widest">เวลา</span>
               </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-3xl border border-slate-100 dark:border-slate-800">
             <div className="flex items-center gap-2 text-slate-400 mb-1">
               <User size={14} />
               <span className="text-[10px] font-bold uppercase tracking-widest">ผู้แจ้ง</span>
             </div>
          ...
          <a
            ...
            className="flex items-center justify-center gap-3 p-4 bg-blue-500 hover:bg-blue-600 text-white rounded-3xl shadow-lg shadow-blue-500/30 transition-all active:scale-95"
          >
            <Navigation size={20} />
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {incident.status === 'RESOLVED' && (
              <div className="text-center p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-3xl border border-emerald-100 dark:border-emerald-900/20">
                <p className="text-emerald-600 font-bold text-sm">✓ This operation has been completed.</p>
              </div>
            )}
          </div>
        )}
